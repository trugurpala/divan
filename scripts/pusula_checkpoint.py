#!/usr/bin/env python3
"""Token-friendly continuity capsules for long-running Pusula development.

The module deliberately uses only the Python standard library so it can run in the
same constrained environments as the existing Divan verification surface.
"""
...
def _normalized_body(data: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if checkpoint < 100 and not next_actions:
...
def _command_seal(args: argparse.Namespace) -> int:
    sealed = seal_capsule(_load(args.input))
...
def _command_validate(args: argparse.Namespace) -> int:
    capsule = validate_capsule(_load(args.path))
    print(
        json.dumps(
            {
                "status": "valid",
...
def _command_render(args: argparse.Namespace) -> int:
    rendered = render_markdown(_load(args.path))
