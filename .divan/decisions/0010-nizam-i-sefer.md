...
Nizam-i Sefer is a stdlib-only `planning.py` component owned by the existing
`council` module. It enriches the schema-2 intent route; it is not a tenth
module, model runtime, daemon, external swarm, or separate repository.
...
The planner never recommends more than three parallel workstreams. Conflicting
host hints become `ambiguous`. Host hint values are never persisted. External
agent harnesses remain unnecessary; parallel writes still require explicit
isolation and ownership under Ordu Nizami.
