...
outside the checkout. PR #29's persistent-memory core is being refreshed on
that main state. Local integration verification found 514 tests with 11
platform skips before hardening and 517 afterward. The follow-up security
review added locked-state reload, symlink containment, release-surface
tracking, and a three-OS memory smoke.
...
- PR #29 carries the persistent project-memory core. Its current-main
  integration and local security review are complete; replacement PR CI,
  cross-platform memory smoke, and independent review remain merge gates.
...
Publish the current-main refresh to PR #29 and require the three-OS memory
smoke plus all applicable PR checks before merge. Keep issue #34 and v1 at 7/8
until reproducible non-owner evidence exists.
