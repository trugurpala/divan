...
fn probe(id: &'static str, label: &'static str, candidates: &[(&str, &[&str])]) -> ToolProbe {
        ...
        match execute(program, args) {
            Ok(output) if output.status.success() => {
                ...
                return ToolProbe {
                    ...
                    label,
                    available: true,
                    detail: if !stdout.is_empty() { stdout } else if !stderr.is_empty() { stderr } else { "hazır".into() },
                };
            }
            Ok(output) => {
                let stderr = first_line(&output.stderr);
    ...
    ToolProbe {
        ...
        label,
        available: false,
        detail: if last_error.is_empty() { "bulunamadı".into() } else { last_error },
    }
}
...
fn probe_orca() -> ToolProbe {
    match execute("orca", &["status", "--json"]) {
        ...
        Ok(output) => ToolProbe {
            ...
            detail: {
                let stderr = first_line(&output.stderr);
                if stderr.is_empty() { "status başarısız".into() } else { stderr }
            },
        },
        Err(error) => ToolProbe {
            id: "orca",
...
fn runtime_candidates(app: &tauri::AppHandle) -> Vec<PathBuf> {
    ...
    if let Ok(resource_dir) = app.path().resource_dir() {
        candidates.push(resource_dir.join("resources").join("divan-project.pyz"));
        candidates.push(resource_dir.join("divan-project.pyz"));
    }
    candidates.push(
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            ...
            .join("..")
            .join("dist")
            .join("divan-project.pyz"),
    );
    candidates
}

fn find_runtime(app: &tauri::AppHandle) -> Option<PathBuf> {
    runtime_candidates(app).into_iter().find(|candidate| candidate.is_file())
}

fn python_candidates() -> [(&'static str, &'static [&'static str]); 3] {
    [("py", &["-3"]), ("python", &[]), ("python3", &[])]
}

fn run_runtime(app: &tauri::AppHandle, runtime_args: &[String]) -> Result<Value, String> {
    let runtime = find_runtime(app).ok_or_else(|| {
        "Divan runtime bulunamadı. dist/divan-project.pyz üretin veya DIVAN_PROJECT_PYZ ayarlayın.".to_string()
    })?;
...
fn runtime_probe(app: &tauri::AppHandle) -> ToolProbe {
    match find_runtime(app) {
        Some(path) => ToolProbe {
            id: "divan-runtime",
            label: "Divan Runtime",
            available: true,
            detail: path.file_name().and_then(|name| name.to_str()).unwrap_or("divan-project.pyz").into(),
        },
        None => ToolProbe {
            id: "divan-runtime",
            label: "Divan Runtime",
            available: false,
            detail: "divan-project.pyz henüz paketlenmedi".into(),
        },
    }
}
...
fn health_check(app: tauri::AppHandle) -> HealthSnapshot {
    ...
    let tools = vec![git, codex, claude, opencode, orca, python, runtime];
    let status = if tools.iter().any(|tool| tool.id == "git" && !tool.available) {
        "blocked"
    } else {
        "ready"
    };
    HealthSnapshot { schema_version: 1, status, tools }
}
...
fn main() {
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![health_check, project_status, goal_start_preview])
