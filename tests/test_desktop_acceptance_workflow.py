...
class DesktopAcceptanceWorkflowTests(unittest.TestCase):
    ...
    def test_acceptance_environment_must_preexist_with_exact_reviewer_authority(self) -> None:
        ...
        self.assertIn("must prevent workflow initiators", preflight)
        ...
        self.assertIn("must disallow administrator bypass", preflight)
    ...
    def test_acceptance_revalidates_environment_after_gate_before_checkout(self) -> None:
        ...
        self.assertIn("$requiredReviewerRules[0].prevent_self_review -ne $true", policy)
        self.assertIn("must still prevent self-review", policy)
        ...
        self.assertIn("can_admins_bypass", policy)
        self.assertIn("administrator bypass became enabled", policy)
