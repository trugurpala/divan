...
class DesktopReleaseEnvironmentPolicyTests(unittest.TestCase):
    ...
    @staticmethod
    def _assert_strict_policy_block(text: str) -> None:
        self = DesktopReleaseEnvironmentPolicyTests
        if "required_rule_count" in text:
            # GitHub-hosted bash preflight.
            assert 'select(.type == "required_reviewers")' in text
            assert 'required_rule_count' in text
            assert 'reviewer_count' in text
            assert 'prevent_self_review' in text
            ...
            assert 'main_policy_count' in text
        else:
            # Windows post-approval revalidation.
            assert 'Where-Object { $_.type -eq "required_reviewers" }' in text
            assert 'prevent_self_review -ne $true' in text
            assert 'can_admins_bypass -ne $false' in text
            assert 'custom_branch_policies' in text
            assert 'protected_branches' in text
            assert 'deployment-branch-policies' in text
            assert 'Where-Object { $_.name -eq "main" }' in text
    ...
    def test_candidate_has_unprotected_preflight_before_protected_candidate(self) -> None:
        ...
        self.assertIn("if: github.event_name == 'workflow_dispatch' && github.ref == 'refs/heads/main'", preflight)
    ...
    def test_candidate_revalidates_policy_after_approval_before_checkout(self) -> None:
        ...
        verify_start = candidate.index("- name: Re-verify production-release policy after environment approval")
    ...
    def test_promotion_has_unprotected_preflight_before_protected_promotion(self) -> None:
        ...
        self.assertIn("GH_TOKEN: ${{ github.token }}", preflight)
        self.assertNotIn("secrets.", preflight)
        promote_header = self.promotion[promote_start : self.promotion.index("    steps:", promote_start)]
    ...
    def test_promotion_revalidates_policy_after_approval_before_checkout(self) -> None:
        ...
        verify_start = promote.index("- name: Re-verify production-release policy after environment approval")
    ...
    def test_release_contract_tracks_policy_and_promotion_workflow_changes(self) -> None:
        ...
        self.assertIn('      - "tests/test_desktop_release_environment_policy.py"', self.candidate)
