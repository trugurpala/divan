...
- [x] PR #17/#16 birleşme kanıtını yeniden okuyup temiz eski worktree ve yerel/
  uzak dalları kaldır.
...
- [x] ADR, ilerleme ve yerel teftiş kanıtını tamamla; v1 7/8 durumunu değiştirme.

### Görev 5: İnceleme ve kamusal teslim

- [x] AGENTS.md'deki tam doğrulama zincirini, Ruff C90, mypy, coverage ve
  `git diff --check` ile birlikte taze çalıştır.
- [x] Bağımsız reviewer ajanına base/head diff'ini incelet; önemli bulguları düzelt.
- [x] Dalı commit/push et, PR aç, CI'yı izle ve yeşilse `main`e birleştir.
- [x] `main` üzerindeki CI, tag/Release, Pages ve Wiki kanıtını doğrula.
- [x] Sabit v0.12.1 kaynağını Claude ve Codex'e dry-run ardından global kur;
