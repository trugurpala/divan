...
- [ ] Bozuk UTF-8, BOM, mojibake ve allowlist cache tespiti için başarısız
  birim testlerini yaz ve beklenen nedenle kırmızı olduklarını gör.
- [ ] `--check` ve güvenli `--clean` davranışını en küçük kodla uygula.
- [ ] Validator entegrasyon testini önce kırmızı, sonra yeşil yap.
- [ ] Birinci taraf subprocess/dosya yollarını açık UTF-8'e geçir.
...
- [ ] C90 + `max-complexity = 25` sözleşmesini isteyen başarısız testi yaz.
- [ ] Kalite yapılandırmasını ve CI komutunu ekle; Ruff'ın üç gerçek ihlali
  yakaladığını kaydet.
- [ ] `validate.denetle`, `_validate_real_agent_evidence` ve
  `rollback_transaction` fonksiyonlarını davranış değiştirmeden parçala.
- [ ] İlgili test kümeleri, Ruff C90 ve mypy'ı yeşile getir.

### Görev 3: Kanıtlı kalıcı temizliği uygula

- [ ] `python scripts/hijyen.py --clean` ile repo cache'lerini sil.
- [ ] PR #17/#16 birleşme kanıtını yeniden okuyup temiz eski worktree ve yerel/
  uzak dalları kaldır.
- [ ] Aktif Divan yedeğinde yalnız `__pycache__` dizinlerini kaldır; manifestin
  işaret ettiği `ui-ux-pro-max` geri yükleme ağacını koru.
- [ ] Temizlik sonrası `--check`, Git worktree ve manifest envanterini doğrula.
...
- [ ] `scripts/yayin.py` yolu ile v0.12.1'i hazırla; yeni hijyen yüzeyini yayın
  manifestine ekle.
- [ ] Kullanıcıya güvenli temizlik ve korunan yedek ayrımını açıkça belgelet.
...
- [ ] Bağımsız reviewer ajanına base/head diff'ini incelet; önemli bulguları düzelt.
