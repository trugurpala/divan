...
class TaskStore:
    ...
    def materialize_goal(self, project: Path | str, goal_id: str) -> dict[str, Any]:
        """Create idempotent tasks from one receipt-verified Nizam-i Sefer route."""
        ...
        for row in rows:
            ...
            try:
                existing = self.load(expected.task_id)
            ...
            if not _same_materialization(existing, expected):
                raise ValueError(
                    f"materialized task conflicts with existing task: {expected.task_id}"
                )
    ...
    def goal_tasks(self, project: Path | str, goal_id: str) -> dict[str, Any]:
    ...
    def blocking_dependencies(self, task: DivanTask) -> tuple[str, ...]:
        """Return this task's dependency ids that are not merged or released yet.

        Returns a tuple because ``TaskStore.list`` shadows the ``list`` builtin
        inside this class body, and to match the store's other read accessor.
        """
...
def _task_title(row: Mapping[str, Any]) -> str:
